...
float INA219_WE::getCurrent_A(){
    ...
    return ((val - offsetCurrent) / (currentDivider_mA * shuntFactor)/1000.0);
}

float INA219_WE::getBusPower_mWh(){
    uint16_t val;
    float busPwr = 0.0;
    if(offsetIsSet){
...
float INA219_WE::getBusPower_Wh(){
    uint16_t val;
    float busPwr = 0.0;
    if(offsetIsSet){
        float current = getCurrent_mA();
    ...
    return busPwr/1000.0;
}

float INA219_WE::getEnergy(){
    static unsigned long millisNow;
    static unsigned long prevMillis = 0;
    unsigned long timeSpan;
    millisNow = millis();
    if (millisNow - prevMillis >= 1000){
        if (getCurrent_mA()>=0.0){
            timeSpan = (millis()-prevMillis)/1000;
            //Serial.println("TimeSpan : "+String(timeSpan));
            energy += getBusPower_Wh()/3600*timeSpan;
        }
        else{
            energy += 0.0;
        }
...
float INA219_WE::getChargingEnergy(){
    static unsigned long millisNow2;
    static unsigned long prevMillis2 = 0;
    unsigned long timeSpan;
    float power;
    float chargingPower;
    millisNow2 = millis();
    if (millisNow2 - prevMillis2 >= 1000){
        if (getCurrent_mA()<0.0){
            power = getBusPower_Wh();
            chargingPower = sqrt(power*power);
            timeSpan = (millis()-prevMillis2)/1000;
            //Serial.println("TimeSpan : "+String(timeSpan));
            chargingEnergy += chargingPower/3600*timeSpan;
        }
        else{
            chargingEnergy += 0.0;
        }
        prevMillis2 = millisNow2;
    }
